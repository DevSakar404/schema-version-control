...
- [ ] **Step 3:** Write the hazard-through-merge test, the case `design.md` §8 exists for: ours drops `users.id`; theirs adds a foreign key referencing it. Assert `conflicts` is **empty** and `hazards` contains one `dangling_foreign_key` with severity `error`. A merge reporting success here is the exact failure this project exists to prevent.
...
- [ ] **Step 3:** Plant scenario two — a **zero-conflict merge with a hazard**: one branch drops a column, the other adds a foreign key referencing it. The hardest thing the tool does, reachable in one click, with attribution naming both branches.
